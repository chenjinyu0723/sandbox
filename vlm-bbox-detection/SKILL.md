...
description: >
  ...
  coordinates, draws them with Chinese labels (same objects auto-numbered:
  XX1, XX2, ...), saves to a fixed-name JSON ({prefix}_bbox.json). Then the agent
  reads the annotated image back, checks accuracy, adjusts coordinates in-place,
  and repeats until all boxes are confirmed accurate. Fully generic — works with
  any objects the user asks to detect.
triggers:
...
**你就是 VLM。** 直接看图给出 bbox，用户说"标出XX和YY"你就标 XX 和 YY。完全泛用，不限物体类型。
...
```
车间照片.jpg     → 车间照片_bbox.json
test.png        → test_bbox.png   错了！应该是 test_bbox.json
```

### 二、每轮迭代你都必须说清楚

对每个目标物体，逐条说出它的位置、当前 bbox、是否需要调整、往哪调、调多少：

```
XX1：位于图片左下角，当前 bbox [120, 200, 320, 380]
  → 右边界没包住 → 向右扩展 20px → 新 bbox [120, 200, 340, 380]

YY1：位于图片右上角，当前 bbox [510, 160, 690, 290]
  → 框准确，不动
```
...
你是多模态模型，直接看原图，输出 JSON：

```json
{
  "image": {"width": 1920, "height": 1080},
  "objects": [
    {"name": "XX1", "bbox": [120, 200, 320, 380]},
    {"name": "XX2", "bbox": [600, 180, 780, 360]},
    {"name": "YY1", "bbox": [500, 150, 680, 280]}
  ]
}
```

规则：
- name 自带编号：XX1、XX2、YY1...
- bbox 用 `[x1, y1, x2, y2]` 像素坐标
- 用户说"标出 XX 和 YY"，你只标 XX 和 YY，不标别的
- 找不到的物体不写
...
| 参数 | 说明 |
|------|------|
| `--image` | 原始图片路径（画框底图，始终用原图） |
| `--json` | bbox JSON 文件路径 |
| `--output` | 标注图输出路径 |
| `--json-output` | JSON 输出路径（通常和 --json 同一个文件） |
  ...
  "objects": [
    {
      ...
      "iterations": [
        ...
        {"round": 2, "bbox": [115, 195, 325, 385], "feedback": "右边界收"},
