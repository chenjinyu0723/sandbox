...
class PptxConverter:
    ...
    def __init__(self):
        ...
        self._raw_bytes: Optional[bytes] = None  # 原始 PPTX 字节（用于读取 mc:AlternateContent）
    ...
    def _convert_package_stream(self, file_stream: BinaryIO) -> None:
        ...
        # 保存原始字节供 _extract_alternate_content_shapes 使用
        if self._raw_bytes is None:
            self._raw_bytes = read_stream_bytes_from_start(file_stream)
    ...
    def _walk_linear(self, pptx_obj: presentation.Presentation):
        ...
        for _, slide in enumerate(pptx_obj.slides):
            ...
            # 提取 <mc:AlternateContent> 中被 python-pptx 隐藏的 shape，
            # 例如嵌入在图表兼容性回退内容中的公式 (OMML)。
            try:
                self._extract_alternate_content_shapes(slide)
            except Exception:
                pass
    ...
    def _collect_shape_blocks(
        ...
        try:
            ...
            if not getattr(shape, "has_text_frame", False):
                # Some equation / math shapes don't expose has_text_frame
                # but still contain OMML markup.  Extract equations from
                # the raw XML before falling through.
                if self._extract_equations_from_shape_xml(shape):
                    return shape_blocks
                return shape_blocks
    ...
    def _extract_equations_from_shape_xml(self, shape) -> bool:
        """Extract OMML equations from shapes that lack has_text_frame.

        PowerPoint shapes containing embedded Equation / MathType objects or
        certain OMML equations don't set ``has_text_frame=True`` in
        python-pptx, so ``_collect_shape_blocks`` would silently drop them.
        This method scans the raw shape XML for ``m:oMath`` / ``m:oMathPara``
        elements and converts them to EQUATION blocks.

        Returns ``True`` if at least one equation was found and added to
        ``self.cur_page``.
        """
        shape_xml = getattr(shape, '_element', None)
        if shape_xml is None:
        ...
        omath_elements = shape_xml.findall(
            f'.//{{{OMML_NS}}}oMath',
        )
        ...
        for omath in omath_elements:
            try:
                ...
                logger.debug(
                    f'Failed to convert PPTX shape OMML equation to LaTeX: {exc}'
                )
                continue
    ...
    def _extract_alternate_content_shapes(self, slide) -> None:
        """从 ``<mc:AlternateContent>`` 中提取被 python-pptx 隐藏的 shape。

        python-pptx 不解析 ``<mc:AlternateContent>`` 内的 shape。本方法通过
        原始 ZIP 字节重新读取 slide XML，将 ``mc:`` 前缀替换为 dummy 以避免
        lxml 的 markup-compatibility 处理丢弃内容，再按阅读顺序提取文本和公式。
        """
        import zipfile
        from io import BytesIO
        ...
        slide_partname = slide.part.partname
        zip_path = slide_partname.lstrip("/")

        try:
            with zipfile.ZipFile(BytesIO(self._raw_bytes)) as z:
                raw_xml = z.read(zip_path)
        except Exception:
            return

        # 将 mc: 命名空间替换为 dummy，防止 lxml 按 markup-compatibility
        # 规范丢弃 AlternateContent 内的元素
        cleaned = raw_xml.replace(
            b'xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006"',
            b'xmlns:dummy="http://dummy.ns/mc"',
        )
        for tag in (b"AlternateContent", b"Choice", b"Fallback"):
            cleaned = cleaned.replace(b"mc:" + tag, b"dummy:" + tag)
            cleaned = cleaned.replace(b"/mc:" + tag, b"/dummy:" + tag)
        ...
        try:
            ...
            root = etree.fromstring(
                ...
                parser=etree.XMLParser(
                    resolve_entities=False, remove_blank_text=False,
                    huge_tree=True,
                ),
            )
        except Exception:
        ...
        DUMMY_NS = "http://dummy.ns/mc"

        # 找到 AlternateContent → Choice → p:sp
        for ac in root.findall(".//{%s}AlternateContent" % DUMMY_NS):
            for choice in ac.findall("{%s}Choice" % DUMMY_NS):
                for sp in choice.findall("{%s}sp" % P_NS):
                    blocks = self._parse_nested_sp_to_blocks(
                        sp, A_NS, OMML_NS
                    )
                    self.cur_page.extend(blocks)

    def _parse_nested_sp_to_blocks(self, sp, A_NS: str, OMML_NS: str) -> list:
        """将嵌套 ``<p:sp>`` 解析为 MinerU block 列表，保留阅读顺序。

        遍历所有 ``<a:p>`` 段落，对每个段落按 ``<a:r>`` / ``<a14:m>/<m:oMath>``
        的顺序提取文本和公式，合并输出。
        """
        A14_NS = "http://schemas.microsoft.com/office/drawing/2010/main"
        blocks = []
        for para in sp.findall(".//{%s}p" % A_NS):
            parts_text = []
            has_formula = False
            has_text = False
            ...
            for child in para:
                ...
                # <a14:m> 包装器内的 <m:oMath>
                if tag == "{%s}m" % A14_NS:
                    omath = child.find("{%s}oMath" % OMML_NS)
                    if omath is not None:
                        has_formula = True
                        try:
                            latex = str(oMath2Latex(omath)).strip()
                        ...
                        if latex:
                            parts_text.append(
                                self.equation_bookends.format(EQ=latex)
                            )
                    continue

                # 直接子元素中的 <m:oMath>（OMML 不在 a14:m 包装器中时）
                if "oMath" in tag and "oMathPara" not in tag:
                    has_formula = True
                    try:
                        latex = str(oMath2Latex(child)).strip()
                    except Exception:
                        latex = ""
                    if latex:
                        parts_text.append(
